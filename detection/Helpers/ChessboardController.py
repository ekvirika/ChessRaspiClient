...
class ChessboardController:
    ...
    def make_move(self, isCapture):
        uci_move = self.queue.get
...
# def listen_for_moves(chessboard_controller):
#     """Listens for UCI moves sent via sockets and executes them."""
...
#         print(f"Listening for moves on port {port}...")
...
#                 while True:
#                     try:
#                         # Receive UCI move from the client
#                         uci_move = conn.recv(4).decode().strip()
#                         isCapt = conn.recv(1).decode().strip()
                        
#                         if not uci_move:  # If no data is received, break the loop
#                             print("No data received. Closing connection.")
#                             break
                        
#                         print(f"Received move: {uci_move}")
#                         # Execute the received UCI move on the chessboard
#                         chessboard_controller.make_move(uci_move, isCapt)
                    
#                     except socket.timeout:
#                         print("Connection timed out. Waiting for the next move.")
#                         break  # Move to the next connection
#                     except ConnectionResetError:
#                         print("Client disconnected. Waiting for a new connection.")
#                         break


    def run_chessboard_controller(self):
